...
cpdef np.ndarray polygamma2(int n, np.ndarray x, np.ndarray d):
    cdef np.ndarray fac2, n_arr, x_arr
...
# def digamma3(x):
cpdef np.ndarray digamma3(np.ndarray x):
    return np.vectorize(digamma2)(x)

cpdef double digamma2(np.float64_t x):
  cdef double value, euler_mascheroni, r
#  Check the input.
#
  if ( x <= 0.0 ):
    value = 0.0
...
cpdef tuple tau_function_gradient(np.ndarray[np.float64_t, ndim=1] x, dict args):
    ...
    cdef np.ndarray func, ff, Df, df, alpha, beta, val_A, val_B, val_T, pi_val
...
cpdef tuple tau_function_gradient_hessian(np.ndarray[np.float64_t, ndim=1] x, dict args):
    ...
    cdef np.ndarray func, ff, Df, df, hf, hess, Hf, alpha, beta, val_A, val_B, val_T, pi_val
